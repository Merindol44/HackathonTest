...
| ~03:35 | — | — | — | — | — | — | — | **New: fall-harness support** (`--harness 0..1`): virtual pelvis support (vertical PD to 0.78 m + torso righting moments) with a per-Newton usage penalty so the policy learns to support itself; anneal to 0. Smoke test: harness=1.0 turns a dumb constant lunge into 500 upright steps covering +3.76 m (vs 37-step fall at 0.0). This gives PPO the long episodes it needs to discover walking. |
