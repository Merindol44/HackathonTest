...
| ~02:05 | probe | v1 3×0.06 m, Stage B 600k ckpt | — | 86.2 | 75 | 0/10 | — | **Key diagnosis:** deterministic rollouts reach only max pelvis_x = 0.16–0.54 m — the robot never gets near the stairs (x=1.2 m). The failure is not stair technique but a fragile walking foundation: the policy lunges ~0.5 m then falls. Pure PPO can't discover sustained walking here — the lunge is a strong local optimum. Pivoting to an imitation prior (Merlin's idea c). |
